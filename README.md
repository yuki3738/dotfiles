...
- 🚀 ワンコマンドでの環境構築
- 🔧 Zsh + Oh My Zsh によるシェル環境
- 📦 anyenv による複数言語のバージョン管理
- 🔍 ghq によるリポジトリ管理
- ⚡ tmux + Vim による効率的な開発環境
- 🤖 Claude Code のグローバル設定・スキル・エージェント管理
...
- **テーマ**: fino
- **プラグイン**: git, ruby, osx, bundler, rails, themes
...
### AI / CLI ツール
- **[Claude Code](https://docs.anthropic.com/en/docs/claude-code)**: カスタムスキル・エージェント・自動化スクリプト（[詳細](.claude/README.md)）
- **bun**: JavaScriptランタイム・パッケージマネージャ
- **LM Studio**: ローカルLLM実行環境
- **lazygit**: TUI Gitクライアント（`lg` エイリアス）
...
`.zshrc`を編集して、エイリアスや関数を追加できます。`.zsh.d/`ディレクトリにはOh My Zshのカスタム設定（関数、履歴、ディレクトリステータス）が配置されています。
