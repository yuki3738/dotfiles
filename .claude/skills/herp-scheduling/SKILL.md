...
- **南谷 祐貴 (本人)**: `U-L0EK3` — `--create-by` / `--attendee-ids` のデフォルト
- **Simple担当 (アシスタント)**: `U-6X9X2`
- **藤原 悠士朗 / Yujiro Fujiwara** (`f2@mov.am`, role: assistant): `U-3Z5GK`
  - 注意: `U-OOGEW` の **藤原裕斗** (`y.fujiwara@mov.am`, role: guest) は別人でコンタクト作成権限なし。「藤原さんで」と指定された場合は実名・メアドで判別すること

> HERP Public APIの `/v1/users` は1ページ100件で、`page` パラメータでページング可能（`?page=2`, `?page=3` ...）。古参ユーザーは `page=1` に出てこないことがある。`--list-users`（page=1のみ）で見つからない場合は、必ず以下のように page=2 以降を巡回すること：
>
> ```bash
> for page in 1 2 3 4 5 6 7 8; do
>   curl -sS -H "Authorization: Bearer $HERP_API_KEY" \
>     "https://public-api.herp.cloud/hire/v1/users?page=${page}" \
>     | jq -r '.users[]? | select(.name | contains("検索したい名前")) | "\(.id) \(.name) \(.email) \(.role)"'
> done
> ```
>
> `email` パラメータは存在しないためクライアント側で `jq 'select(.email == "...")'` でフィルタする。`role`（admin/recruiter/assistant/guest）と `status`（active/deactivated）での絞り込みは可能。
...
- **候補者の実姓が判別できない場合**（Findyハンドル等）、宛名行は省略して「はじめまして。」から始める。「ご担当者様」「採用ご担当者様」など**応募者→企業向けの呼びかけは絶対に使わない**（企業→候補者の文脈では失礼にあたる）
- 社名は「株式会社mov」（前株）
